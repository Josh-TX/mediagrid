...
//   - videos idle (tilePreviewAlways=false and not hovering): thumbnail if
//     available, else a placeholder (never the original — that'd defeat the
//     point of thumbnails existing).
//   - videos with playback active (tilePreviewAlways=true, or hovering):
//     highlight if available; else the original if fallbackToOriginal;
//     else fall back to the idle state (thumbnail/placeholder).
export function resolveTileSource(input: TilePlaybackInput): TileSource {
  if (!input.isVid) {
    return input.hasThumbnail ? 'thumbnail' : 'original'
  }

  const playbackActive = input.tilePreviewAlways || input.hovering
  if (playbackActive) {
