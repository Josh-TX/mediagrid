...
Renames the `rewindSeconds` and `fastForwardSeconds` preset fields to `rewindAmount` and `fastForwardAmount` everywhere (types, backend DB, frontend). Adds two new boolean preset fields `isRewindPercent` and `isForwardPercent` (default false) that switch seek behavior from fixed seconds to a percentage of video duration (`Math.round(duration * amount / 100)`), applied to both edge taps and arrow keys. The Settings modal gains "Rewind unit type" and "Fast-forward unit type" select inputs (Seconds/Percentage), with the adjacent amount field label updating dynamically. The Player overlay in percent mode shows the computed seconds as usual plus the accumulated percentage in smaller subtle text below. Includes a DB migration to rename the columns and add the two new boolean columns.
